...
export function UiModal({ width = "md", className, children, isOpen = false, onClose }) {
    ...
    const modal = (
        ...
        onClick={handleClick}>
            ...
            <button
                className="w-8 h-8 rounded hover:bg-white/40 transition-colors flex items-center justify-center bg-white/10 absolute top-0 left-[calc(100%+12px)]"
                onClick={onClose}
            >
                <CrossLightIcon className="h-4 w-4 text-white" />
            </button>
                {children}
            </div>
        </div>
...
UiModal.Header = function UiModalHeader({children, className}) {
    return <div className={
        clsx(className, "pt-6 pl-6 pb-4 text-2xl")
    }>
        {children}
    </div>
}
...
UiModal.Footer = function UiModalFooter({children, className}) {
    return <div className={
        clsx(className, "mt-auto p-6 flex gap-4 justify-end")
    }>
        {children}
    </div>
}
